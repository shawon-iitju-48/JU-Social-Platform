...
if (isset($_POST["limit"], $_POST["start"])) {
    ...
    $query = "select *from userpost_photos right join  (select *from userpost where u_id in (select friendto from user_friends where friendfrom=" . $_SESSION["u_id"] . ") order by pid desc limit " . $_POST["start"] . ", " . $_POST["limit"] . ") x  on x.pid=userpost_photos.pid NATURAL join user limit 1";
    $queryv = "select *from userpost_videos right join  (select *from userpost  where u_id in (select friendto from user_friends where friendfrom=" . $_SESSION["u_id"] . ") order by pid  desc  limit " . $_POST["start"] . ", " . $_POST["limit"] . ") x  on x.pid=userpost_videos.pid NATURAL join user limit 1;";

    $query1 = "select count(photosid) as koto from (select *from userpost where u_id in (select friendto from user_friends where friendfrom=" . $_SESSION["u_id"] . ") order by pid desc limit " . $_POST["start"] . ", " . $_POST["limit"] . ")x left join  userpost_photos on x.pid=userpost_photos.pid  group by x.pid ";
    $query2 = "select count(videosid) as koto from (select *from userpost where u_id in (select friendto from user_friends where friendfrom=" . $_SESSION["u_id"] . ") order by pid desc limit " . $_POST["start"] . ", " . $_POST["limit"] . ")x left join  userpost_videos on x.pid=userpost_videos.pid  group by x.pid ";
    ...
    $querydorkar = "select *from userpost where u_id in (select friendto from user_friends where friendfrom=" . $_SESSION["u_id"] . ") order by pid desc limit " . $_POST["start"] . ", " . $_POST["limit"] . "";
    $dorkar = mysqli_query($connect, $querydorkar);
    $kire = mysqli_fetch_array($dorkar);
    if ($kire) {
        $like1 = "select *from userpost_likes where u_id='$uid' and pid=" . $kire['pid'] . "";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        echo '
            <p style="font-size: .8rem;">' . $row["pdate"] . ' at ' . $row["ptime"] . '</p>
        </div>
    </div>
    <p>' . $row["pdetails"] . '</p>
    <div class="divide">';
        ...
        if (!empty($row["location"]))
            echo '
    <a href="post_view.php?pidd=' . $row['pid'] . '"><img id="' . $row['pid'] . $row['pid'] . '" src="' . $row["location"] . '" class="postphoto" alt=""></a>';

        if (!empty($bow["location"]))
            echo '<video id="' . $bow['pid'] . $bow['pid'] . '" src="' . $bow["location"] . '" width="100%" height="320" controls></video>';
    ...
    <div class="like" style="margin-bottom:-.8rem;">
        <div class="left">
            <h6><i style="margin-right:0.5rem;font-weight: 400;color:#2078F4" class="fa-duotone fa-thumbs-up"></i><span style="font-weight:400" id="' . $kire['pid'] . '-' . '">' . $row["likecount"] . '</span>
            </h6>
        </div>
        <div class="right">
    ...
    <button id="' . $kire['pid'] . '_' . '" class="btn ButtonJolo kirebhai" style="width:45%;border-radius:5px;margin-top:-1rem;">';
        ...
        echo '</button>
    <button id="' . $kire['pid'] . '__' . '" class="btn ButtonJolo normi" style="width:45%;border-radius:5px;margin-top:-1rem;"><i style="margin-right:0.5rem;" class="fa-regular fa-message"></i>Comment</button>
    </div>
    <hr>
    <div class="flexx" style="margin-bottom:1rem">
        <a href=""> <img src="' . $_SESSION['dp'] . '" alt=""></a>
        <form autocomplete="off" method="post" id="' . $kire['pid'] . '--' . '">
            <input name="bindas" class="btn btn-light" style="width:25rem;margin-left:1rem;border-radius:40px;text-align: left;cursor:text" placeholder="Write a comment..." id="' . $kire['pid'] . '___' . '" ></input>
