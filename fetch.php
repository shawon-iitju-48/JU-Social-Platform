...
if (isset($_POST["limit"], $_POST["start"])) {
    ...
    $query1 = "select count(photosid) as koto from (select *from userpost order by pid desc limit " . $_POST["start"] . ", " . $_POST["limit"] . ")x left join  userpost_photos on x.pid=userpost_photos.pid where u_id=" . $_SESSION["u_id"] . " group by x.pid ";
    $query2 = "select count(videosid) as koto from (select *from userpost order by pid desc limit " . $_POST["start"] . ", " . $_POST["limit"] . ")x left join  userpost_videos on x.pid=userpost_videos.pid where u_id=" . $_SESSION["u_id"] . " group by x.pid ";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        <div class="posthead">
        <div class="left">
            <a href="profile.php"><img src="' . $_SESSION["dp"] . '" alt=""></a>

        </div>
        <div class="right">
            <a href="profile.php"><b>' . $row["fname"] . ' ' . $row["lname"] . '</b></a> ';
    ...
    <div class="flexx" style="margin-bottom:1rem">
        <a href="peopleprofile.php?pi='.$_SESSION['u_id'].'"> <img src="' . $_SESSION["dp"] . '" alt=""></a>
