...
$resc = mysqli_fetch_array(mysqli_query($con, "select count(no_id) as nnn from (select *from notifications where u_id=" . $_SESSION['u_id'] . " and isseen='no')x inner join user on x.sender=user.u_id order by no_id desc"));
if ($resc['nnn'] > $_SESSION['kotore']) {
    $res = mysqli_query($con, "select *from (select *from notifications where u_id=" . $_SESSION['u_id'] . " and isseen='no')x inner join user on x.sender=user.u_id order by no_id desc limit " . $_SESSION['kotore'] . "," . $resc['nnn'] . "");
