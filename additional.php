...
if (isset($_POST["limitp"], $_POST["startp"])) {
    $gui = "select *from userpost natural join userpost_photos where u_id=" . $_SESSION['u_id'] . " order by pid desc LIMIT " . $_POST["startp"] . ", " . $_POST["limitp"] . "";
    $photo1 = mysqli_query($connect, $gui);
    while ($photo2 = mysqli_fetch_array($photo1)) {
        echo  '<div class="photos">
        <div class="left">
            <a href="post_view.php?pidd=' . $photo2['pid'] . '"><img src="' . $photo2['location'] . '" alt=""></a>
        </div>
    </div>';
    }
}
...
if (isset($_POST["limitf"], $_POST["startf"])) {
    $fr = "select *from user NATURAL join (select friendto as u_id from user_friends where friendfrom=" . $_SESSION['u_id'] . ")x where u_id<>" . $_SESSION['u_id'] . " LIMIT " . $_POST["startf"] . ", " . $_POST["limitf"] . "";
...
if (isset($_POST["limitv"], $_POST["startv"])) {
    $vui = "select *from userpost natural join userpost_videos where u_id=" . $_SESSION['u_id'] . " order by pid desc LIMIT " . $_POST["startv"] . ", " . $_POST["limitv"] . "";
...
if (isset($_POST["omago"])) {
    $kh = "%" . $_POST['omago'] . "%";
    $fr = "select *from user NATURAL join (select friendto as u_id from user_friends where friendfrom=" . $_SESSION['u_id'] . ")x where u_id<>" . $_SESSION['u_id'] . " and (fname like '$kh' or lname like '$kh' or concat_ws(' ',fname, lname) like '$kh');";
    $fr0 = mysqli_query($connect, $fr);
    while ($fr4 = mysqli_fetch_array($fr0)) {
        echo '<div class="friend">
        <div class="left">
